...
namespace Tree
{
    ...
    void insert(int &root,int l,int r,int pos,int val)
    {
        ...
        {
            tree[root].mx+=val;
            if(tree[root].mx>0) tree[root].num=pos;
...
{
    const int N=100010;
    int n,m,cnt,mx,Head[N],root[N],fa[N][20],dep[N],ans[N];
    ...
    void work()
    {
        ...
        for(int i=1;i<=m;i++)
        {
            ...
            Tree::insert(root[u],1,mx,q[i].val,1);
            Tree::insert(root[v],1,mx,q[i].val,1);
            Tree::insert(root[lca],1,mx,q[i].val,-1);
            if(fa[lca][0]) 
                Tree::insert(root[fa[lca][0]],1,mx,q[i].val,-1);
        }
        dfs(1);
        for(int i=1;i<=n;i++)
            printf("%d\n",ans[i]);
