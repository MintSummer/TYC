...
namespace TYC
{
	int n, m, cnt, rt, a[N], val[N], sum[N], size[N], fa[N], son[N][2], pre[N][2], suf[N][2], rev[N], fil[N];
	char ch[N];
	...
	inline void pushdown(int root)
	{
		if (fil[root])
		{
			Filp(son[root][0]), Filp(son[root][1]);
			fil[root] = 0;
		}
		if (rev[root])
		{
	...
	void work()
	{
		...
		while (m--)
		{
			...
			switch (q)
			{
				...
				case 1: Reverse(son[x][0]); break;
				case 2: Filp(son[x][0]); break;
			}
			if (q != 0) update(x), update(fa[x]); 
		}
	}
}
int main()
